...
(* fonction   afficher_grille : afficher une grille *)
...
let (p:probleme)=
 ...
 afficher_grille p;;
(* fct solution_de : verifier si une grille g est une solution de probleme p*)
...
(* Declaration du type hypothese comme tableau à deux dimensions dans chaque case il y'a un tableau à une seule dimension *)
...
h;;
(* fct compatible : verifier si une hypothese h est compatible avec une grille g*)
    ...
    try
        for i = 0 to 8 do
          for j = 0 to 8 do
            ...
            if  !x = 0  then raise Exit  else x := 0 
            )
          done;
        done;
        true;
